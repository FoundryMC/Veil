...
vec3 viewToWorldSpaceDirection(vec3 direction) {
    return (IViewMat * vec4(direction, 0.0)).xyz;
}

vec3 viewToPlayerSpace(vec3 positionVS) {
    return (IViewMat * vec4(positionVS, 1.0)).xyz;
}

vec3 playerSpaceToWorldSpace(vec3 positionPS) {
    return positionPS + CameraPosition;
}

vec3 worldToViewSpaceDirection(vec3 viewSpace) {
    return (ViewMat * vec4(viewSpace, 0.0)).xyz;
}

float depthSampleToWorldDepth(float depthSample) {
    float f = depthSample * 2.0 - 1.0;
    return 2.0 * NearPlane * FarPlane / (FarPlane + NearPlane - f * (FarPlane - NearPlane));
}
...
vec3 viewDirFromUv(vec2 uv) {
    return (IViewMat * vec4(normalize(viewPosFromDepth(1.0, uv)), 0.0)).xyz;
}