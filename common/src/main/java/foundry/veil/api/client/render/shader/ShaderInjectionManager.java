...
import foundry.veil.impl.client.render.shader.injection.ShaderInjectionAdapter;
import foundry.veil.impl.client.render.shader.injection.ShaderInjectionLoader;
...
public class ShaderInjectionManager extends SimplePreparableReloadListener<ShaderInjectionManager.Preparations> {
    ...
    protected @NotNull Preparations prepare(@NotNull ResourceManager resourceManager, @NotNull ProfilerFiller profilerFiller) {
        ...
        loaded.byTarget().forEach((target, patches) -> {
            ...
            for (ShaderInjectionLoader.LoadedPatch patch : patches) {
                if (patch.definition().replace() != null) {
                    ResourceLocation replaceTarget = stripShaderExtension(target);
                    if (replaceTarget == null) {
                        replaceTarget = target;
                    }
                    replacements.put(replaceTarget, patch.definition().replace());
                    if (patch.definition().debug()) {
                        Veil.LOGGER.info("\n═══ Replace: {} ═══\n  Target:   {}\n  Replace:  {}\n  Source:   {}",
                                replaceTarget, target, patch.definition().replace(), patch.resourceLocation());
                    }
                    continue;
                }
                try {
                    for (ShaderInjection m : ShaderInjectionAdapter.toModifications(patch.definition(), resourceManager)) {
                        mods.add(m);
                        names.put(m, patch.definition().id() != null ? patch.definition().id() : patch.resourceLocation());
                    }
                } catch (Exception e) {
                    Veil.LOGGER.error("Couldn't adapt shader injection {}", patch.resourceLocation(), e);
                }
            }
        });
    ...
    protected void apply(@NotNull Preparations preparations, @NotNull ResourceManager resourceManager, @NotNull ProfilerFiller profilerFiller) {
        ...
        if (!this.replacements.isEmpty()) {
            ...
            this.replacements.forEach((target, replacement) ->
                    Veil.LOGGER.debug("  {} -> {}", target, replacement));
        }
    }
