...
public record FramebufferAttachmentDefinition(FramebufferAttachmentDefinition.Type type,
    ...
    public enum Format {
         ...
         * mods like TACZ enable stencil on the main target, which changes it to {@link #DEPTH32F_STENCIL8}.
         * {@code glBlitFramebuffer} requires depth formats to match exactly, so framebuffers that exchange depth with
         * the main render target have to use whatever format that target actually has.</p>
         *
