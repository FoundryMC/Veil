...
Skeletons are the basis of Necromancer, acting as a container for `Bone`s. Skeletons do very little on their own - they require a `Skin` to be rendered, and an `Animator` to be puppeteered.
...
The `buildRoots()` function should be called after adding bones!
...
An `Animator` puppeteers a Skeleton, updating the transformations of bones and the like. It does this through its `animate()` function, as well as applying given animations and constraints each tick.
...
`SkeletonParent` is an interface, providing functionality for attaching a skeleton to an Entity (support for block entities is also planned.) When implemented by an entity, Necromancer will _automatically_ attach a Skeleton and Animator, generated using the factories specified in the entity's NecromancerEntityRenderer.

## NecromancerEntityRenderer

An extension of vanilla's `EntityRenderer`, built for entities implementing SkeletonParent. This automatically handles attaching Skeletons and Animators to entities, drawing the Skeleton through a provided Skin, and entity render layers.

### NecromancerEntityRenderLayer

An equivalent of vanilla's `EntityRenderLayer`, for entities implementing SkeletonParent. Should be added to a NecromancerEntityRenderer in its constructor, using the `NecromancerEntityRenderer.addLayer(layer)` method.
